function [pred,mcpr,mx, my]=blockmatching(f_r,f_t,N,M) 
...
% f_r = mat2gray(prev);  f_t = mat2gray(curr); 
...
whos pred
pred = mat2gray(pred); %% this line added
whos pred
...
%         end
        ...
        pred(start_t_i:end_t_i,start_t_j:end_t_j)=f_r(start_r_i:end_r_i,start_r_j:end_r_j); 
    end
    b=1; 
    a=a+1;
end
...
whos pred
mcpr=imsubtract(pred,f_t); 
pred = char(pred);
prevTemp = imread(pred);
pred = rgb2gray(prevTemp);
whos pred
% whos dfd
...
min_dfd=min(min(mcpr)); 
mcpr=mcpr+abs(min_dfd); 
...
title('Target Image'); 
subplot(1,2,2),imshow(mat2gray(pred)); 
title('Predicted Image'); 
figure,subplot(1,2,1),imshow(mat2gray(mcpr));
...
clearvars -except mcpr pred mx my;
% Reference %%
