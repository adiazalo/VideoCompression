...
imgq_dec_7372800x1 = decArith('dct_hist',bitfile);
imgq_dec_1x7372800 = imgq_dec_7372800x1';
%% inverse shifting
imgq_dec_115200x64 = reshape(imgq_dec_1x7372800',[64,115200])';
imgq_dec_115200x64 = imgq_dec_115200x64 - 1;
imgq_dec_115200x64 = imgq_dec_115200x64 - round(abs(min_index));
%% inverse DICM
for rIndex = 115200:-1:2
    imgq_dec_115200x64(rIndex,1) = imgq_dec_115200x64(rIndex,1) + imgq_dec_115200x64(rIndex-1,1);
end
%% inverse DCT
tempBlock8x8 = ones(8);
img_dec = zeros(1920,3840);
rIndex = 1;
cIndex = 1;
nextBlockNum = 1;
vec = ones(1,64);

vecq = imgq_dec_115200x64(nextBlockNum,:);
while rIndex<1920
    while cIndex<3840
        vecq = imgq_dec_115200x64(nextBlockNum,:);
        for qIndex = 1:64
